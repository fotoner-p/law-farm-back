...
class CRUDForumLike(CRUDBase[ForumLike, ForumLikeCreate, ForumLikeUpdate]):
    ...
    def remove_with_owner_and_forum(self, db: Session, *, forum_id: int, owner_id: int):
        obj = db.query(self.model).filter(self.model.owner_id == owner_id).filter(self.model.forum_id ==forum_id).first()
        db.delete(obj)
        db.commit()
        return obj
